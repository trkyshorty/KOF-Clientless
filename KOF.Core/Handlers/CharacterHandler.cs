...
public class CharacterHandler : IDisposable {
    ...
    public Character ParseOther(Message msg, bool isGroup) {
        ...
        var loop = MySelf.LunarWarDressUp ? 9 : 16;
    ...
    private Task ProcessAttack() {
        try {
            ...
            if (!Controller.GetControl("Attack", false))
                return Task.CompletedTask;
            if (GetGameState() != GameState.GAME_STATE_INGAME || IsUntouchable() || IsRouting())
                return Task.CompletedTask;
            if (MySelf.IsTrading)
                return Task.CompletedTask;
    ...
    private Task ProcessTargetAndAction() {
        try {
            ...
            if (!Controller.GetControl("Attack", false))
                return Task.CompletedTask;
            if (GetGameState() != GameState.GAME_STATE_INGAME || IsRouting())
                return Task.CompletedTask;
            if (MySelf.IsTrading)
                return Task.CompletedTask;
    ...
    public void PartySend(Character target) {
        if (target.Party.IsInParty())
            return;
        if (MySelf.Zone != target.Zone)
            return;
    ...
    public void PartyPromoteLeader(int memberId) {
        if (!MySelf.Party.IsInParty())
            return;
        if (MySelf.Party.Leader.Name != MySelf.Name)
            return;
    ...
    public void ItemBuy(int npcId, int itemSellingGroup, int ItemId, short itemCount) {
        ...
        foreach (KeyValuePair<int, List<int>> shopItem in shopItemListDict) {
            if (!shopItem.Value.Any(x => x == ItemId))
                continue;
            ...
            case RouteActionType.SUPPLY: {
                if (MySelf.GetPosition() != movePosition && route.SubQueue.Count == 0)
                    ...
                    if (route.SubQueue.Count == 0) {
                        ...
                        var potion = GetNpcList().FirstOrDefault(x => x.Name.Contains("Potion", StringComparison.InvariantCultureIgnoreCase) || x.FamilyType == 21);
    ...
    public void LoadQuestList() {
        ...
        questHelper.ForEach(x => {
            ...
            if (MySelf.Level < questGuide.MinLevel)
                return;
            if (MySelf.ActiveQuestList.Any(y => y.Id == x.EventDataIndex && (y.Status == 2 || y.Status == 1)))
                return;
